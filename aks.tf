data "azurerm_virtual_network" "aks_core_vnet" {
  ...
  name                = var.env == "demo" || var.env == "prod" ? "core-${local.env}-vnet" : "cft-${local.env}-vnet"
  resource_group_name = var.env == "demo" || var.env == "prod" ? "aks-infra-${local.env}-rg" : "cft-${local.env}-network-rg"
}
