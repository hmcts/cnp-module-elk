...
resource "azurerm_template_deployment" "alert_cluster_health_not_green" {
  ...
  parameters = {
    ...
    SearchQuery            = "es_health_CL | where (status_s != \"green\" and cluster_s == \"${azurerm_resource_group.elastic-resourcegroup.name}\") or error_type_s != \"\" | where not ((datetime_part(\"hour\", TimeGenerated) == 5 and datetime_part(\"minute\", TimeGenerated) >= 30 and datetime_part(\"minute\", TimeGenerated) <= 50) or (datetime_part(\"hour\", TimeGenerated) == 17 and datetime_part(\"minute\", TimeGenerated) >= 0 and datetime_part(\"minute\", TimeGenerated) <= 10))"
    Severity               = "warning"
    TimeWindow             = "10"
    AlertFrequency         = "5"
