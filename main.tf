...
resource "null_resource" "dynatrace_oneagent_networkzone" {
  ...
  provisioner "local-exec" {
    command = "env AZURE_CONFIG_DIR=/opt/jenkins/.azure-${var.subscription} az vm extension set --name oneAgentLinux​ --publisher dynatrace.ruxit​​ --resource-group ${azurerm_resource_group.elastic-resourcegroup.name}​ --vm-name "${var.product}-data-${count.index}"​ --settings "{\"tenantId\":\"${var.dynatrace_instance}\",\"token\":\"${var.dynatrace_token}\", \"installerArgs\":\"--set-host-group=${var.dynatrace_hostgroup} --set-infra-only=false --set-network-zone=azure.cft\"}""
  }
...
resource "null_resource" "azcli_exec" {
  provisioner "local-exec" {
    command = "env AZURE_CONFIG_DIR=/opt/jenkins/.azure-${var.subscription} az vm extension set​ --name oneAgentLinux --publisher dynatrace.ruxit​​ --resource-group ${azurerm_resource_group.elastic-resourcegroup.name}​ --vm-name "${var.product}-kibana"​ --settings "{"tenantId":"${var.dynatrace_instance}","token":"${var.dynatrace_token}", "installerArgs":"--set-host-group=${var.dynatrace_hostgroup} --set-infra-only=false --set-network-zone=azure.cft"}""
  }
